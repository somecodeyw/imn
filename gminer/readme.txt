...
'--zilmt' - space-separated list of memory tweak numbers for each device for ZIL mining round (range from 0 to 6, 0 - disable tweaks), only Nvidia GPUs with GDDR5X and GDDR5 memory are supported, requires running miner with admin privileges (for example: '1 3 5')
'--zilpl' - space-separated list of power limits for each device in percents for ZIL mining round (range from 0 to 100 for Nvidia GPUs and -50 - 50 for AMD GPUs, 0 - ignore)(for example: '30 0 50')
'--zilcclock' - space-separated list of core clock offsets (for Nvidia GPUs) or absolute core clocks (for AMD GPUs) for each device in MHz (0 - ignore) for ZIL mining round, requires running miner with admin privileges (for example: '100 0 -90')
'--zilmclock' - space-separated list of memory clock offsets (for Nvidia GPUs) or absolute memory clocks (for AMD GPUs) for each device in MHz (0 - ignore) for ZIL mining round, requires running miner with admin privileges (for example: '100 0 -90')
'--zilcvddc' - space-separated list of core voltage offsets in % (for Nvidia GPUs) or absolute core voltages (for AMD GPUs) for each device in mV (0 - ignore) for ZIL mining round, only Windows is supported, requires running miner with admin privileges (for example: '900 0 1100')
'--zillock_voltage' - space-separated list of locked voltage points for each device in mV (0 - ignore) for ZIL mining round, only Windows and Nvidia GPUs are supported. Requires running miner with admin privileges (for example: '900 0 1000')
'--zillock_cclock' - space-separated list of locked core clock point for each device in MHz (0 - ignore) for ZIL mining round, only Nvidia GPUs are supported. Requires running miner with admin privileges (for example: '1200 0 1500')
'--zillock_mclock' - space-separated list of locked memory clock point for each device in MHz (0 - ignore) for ZIL mining round, only Nvidia GPUs are supported. Requires running miner with admin privileges (for example: '810 0 1000')
...
'--dalgo' - dual mining algorithm (for example: 'kaspa')
'--dserver' - dual stratum server address (for example: 'eu1.zhash.pro')
'--dport' - dual stratum server port (for example: '5057', '1445')
'--duser' - dual stratum server username (for example: 'sRuJK1BmA758GbOn', 'GfGLyfP9GzZbPeTzvW1KSx3HeMnrNAiGWY')
'--dpass' - dual stratum server password, default value is 'x' (for example: 'sRuJK1Bm')
'--dssl' - enable/disable ssl for dual stratum connection ('0' - off or '1' - on), must be supported by a pool, default value is '0' 
'--dworker' - dual worker name for pools that does not supoort wallet.worker (for example: 'rig0')
'--dual_intensity' or shortly '-di' - space-separated list of intensities for dual mining [0 - 128]
'--zilserver' - ZIL stratum server address (for example: 'eu1.zhash.pro')
'--zilport' - ZIL stratum server port (for example: '5057', '1445')
'--ziluser' - ZIL stratum server username (for example: 'sRuJK1BmA758GbOn', 'GfGLyfP9GzZbPeTzvW1KSx3HeMnrNAiGWY')
'--zilssl' - enable/disable ssl for ZIL stratum connection ('0' - off or '1' - on), must be supported by a pool, default value is '0' 

Dual examples:
'miner --algo ergo+kaspa --server erg.2miners.com:8888 --user 9hcksqJGwwYP6hwxzs6fAqfu3Qdi7YdHoK9GrUxpYhNDeFCZrPh --dserver pool.eu.woolypooly.com:3112 --duser kaspa:qq2yknr3t9nlp0knp954lua4xkregc88g0nxkag7c82rj4mw3v0v5nfjsjgfc'
'miner --algo ethash --server ethw.2miners.com:2020 --user 0x00192Fb10dF37c9FB26829eb2CC623cd1BF599E8 --dalgo kaspa --dserver pool.eu.woolypooly.com:3112 --duser kaspa:qq2yknr3t9nlp0knp954lua4xkregc88g0nxkag7c82rj4mw3v0v5nfjsjgfc'

Parameters dag_mode, safe_dag, dag_limit, kernel, mt, fan, pl, cclock, cvddc, mclock, lock_voltage, lock_cclock, tfan, templimit, templimit_mem, intensity, dual_intensity can be specified with one parameter for all devices:
...
ergo, autolykos2         2%
kaspa, kheavyhash        1%
ethw+kaspa, ethash+kheavyhash3%
etc+kaspa, etchash+kheavyhash3%
